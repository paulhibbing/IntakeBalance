...
The goal is to make these tools and methods as accessible as possible. That
said, we can currently only provide support for using them in R. We understand
that most potential users are not R programmers, so we aim to provide thorough
instructions that will demystify the process and limit how intimidating it
feels. However, we realize things can always be clearer. If you find anything
confusing, please feel free to point it out to us by
...
IntakeBalance::IntakeBalance(
  ...
  ## but now the values reflect the names of columns where that information is
  ## stored, rather than passing in the values themselves
  fm_start = "fm_start",
  fm_end = "fm_end",
  ffm_start = "ffm_start",
  ffm_end = "ffm_end",
...
#### 1. Reading and pre-formatting the wearable data
...
## Extend the data file for the sake of illustration (ignore these commands)
ex_data <- data.frame(
...
use syntax just like the above to acquire that information. For example, you
...
For this example, though, we don't have a built in `agd` file. So instead, we'll
just make up some activity count data. (You're welcome to run the below code if
you're following along with this example, but in an actual application, you
would use an `agd` file rather than completing this step.)
...
#### 2. Finishing the calculations

The previous step gave us estimates of daily energy expenditure (kcal/kg) during
awake wear time. From here, we need to bring in our body composition data and
make a couple of calculations to determine EI. Almost there!

```{r, merge}

## First, let's get our demographic data. We'll pretend it's the first row of
## data from our previous data frame, and we'll assign it an ID label of `001`
## and a body mass of 87 kg

  demographics <- info[1, ]
  demographics$PID <- "001"
  demographics$wt_kg <- 87

## Next, let's determine a Schofield prediction of basal metabolism to impute
## during periods of sleep/non-wear

  demographics$schofield <- PAutilities::get_ree(
    method = "schofield_wt_ht", sex = "M", age_yr = 25, wt_kg = 87,
    ht_m = 1.85, output = "kcal_day", RER = 0.85
  )


## It's time to merge the demographic and energy expenditure data

  ei_data <- merge(demographics, ee_data)
  

## Next, we account for non-wear/sleep time by adding Schofield estimates for
## those periods. We'll use dplyr here as well
  ...
  ei_data %>%
    ...
    group_by(PID = "001") %>%
    summarise(across(where(is.numeric), mean, .names = "{gsub(\"^total\", \"mean\", .col)}"))
